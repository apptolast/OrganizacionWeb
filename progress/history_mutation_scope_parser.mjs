...
const files = ["App.tsx", "workspace.tsx", "project-reader.tsx", "task-reader.tsx"];
const evidence = [];
for (const file of files) {
  ...
  function visit(node) {
    if (file === "App.tsx") {
      if (ts.isVariableDeclaration(node) && node.name.getText(source) === "history") add(node, "history route recognition");
      if (ts.isConditionalExpression(node) && node.condition.getText(source) === "history") {
        add(node, "complete history conditional");

      }
    } else if (ts.isJsxElement(node) && node.openingElement.tagName.getText(source) === "RouteLink" && node.getText(source).includes("/historial")) add(node, "history link");
    ts.forEachChild(node, visit);
  }
  visit(source);
  evidence.push({path, sha256: crypto.createHash("sha256").update(text).digest("hex").toUpperCase(), nodes});
}
for (const file of ["history-api.ts", "history.tsx"]) {
 const path = `frontend/src/${file}`;
 const text = fs.readFileSync(path, "utf8");
...
fs.writeFileSync("progress/history_mutation_scope_nodes.json", JSON.stringify(evidence, null, 2));
