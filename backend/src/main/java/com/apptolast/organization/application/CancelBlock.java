...
public final class CancelBlock implements CancelBlockUseCase {
  ...
  public CancelBlock(BlockEditing store, Clock clock) {
    this.store = store;
    this.clock = clock;
  }

  public BlockChangeConfirmation cancel(
      String owner, UUID project, UUID task, UUID block, UUID key, long expected) {
    return store.cancel(
        owner,
        project,
        task,
        ...
        prior -> {
          ...
          var receipt =
              new BlockChangeReceipt(
                  UUID.randomUUID(), block, "CANCELLED", next.version(), now, prior.block(), null);
          var event =
              new BlockChanged(
                  UUID.randomUUID(),
                  project,
                  ...
                  "CANCELLED",
                  next.version(),
                  BlockChanged.Interval.from(prior.block()),
                  null);
