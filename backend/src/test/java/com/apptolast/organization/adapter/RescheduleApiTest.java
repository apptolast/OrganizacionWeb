...
class RescheduleApiTest {
  ...
  void seed() {
    ...
    jdbc.update(
        ...
        project);
    jdbc.update(
        "INSERT INTO planned_blocks(id,project_id,task_id,request_key,objective,start_local,end_local,zone_id,start_offset,end_offset,allow_over_budget,start_at,end_at,duration_minutes,created_at) VALUES (?,?,?,?,'Preparar borrador',TIMESTAMP '2030-01-07 10:00',TIMESTAMP '2030-01-07 11:00','UTC','Z','Z',false,TIMESTAMPTZ '2030-01-07 10:00Z',TIMESTAMPTZ '2030-01-07 11:00Z',60,TIMESTAMPTZ '2030-01-06 10:00Z')",
        block,
        project,
        task,
        key);
  ...
  void s3_s4_readsCancelledProjectionWithExactLargeRevisionAndLastBlock() throws Exception {
    jdbc.update(
        "INSERT INTO block_projections(block_id,version,status,updated_at) VALUES (?,9007199254740993,'cancelled',TIMESTAMPTZ '2030-01-07 08:59:59.123456Z')",
        block);
    var response =
        mvc.perform(get(base() + "/" + block + "/state").with(user("persona-a")))
            .andExpect(status().isOk())
