...
- La feature está `in_progress` en `feature_list.json`. Si está `pending` o
  `spec_ready`, paras — el `craftsman_lead` no debió lanzarte.
...
7. Si el `craftsman_lead` te reinvoca con el veredicto aprobado y la mutación
   superada: cambia el status a `done` y mueve el resumen a
   `progress/history.md`.
