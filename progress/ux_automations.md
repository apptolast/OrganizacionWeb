...
| --- | --- | --- |
| 1. Anchos 320, 768, 1280 y 1440 px CSS | `e2e/automations-ux.spec.mjs`, **estado vacío más editor abierto**: el `clearRules()` del `beforeEach` deja cero reglas y la simulación no se ejecuta, así que ni el `<ul aria-label="Reglas">` ni el bloque de coincidencias llegaron a pintarse | Sin desplazamiento **horizontal de página** (`documentElement.scrollWidth <= clientWidth`) y todo control ≥ 44 × 44 px. **Medido tambien el recorte de contenido** por elemento y en los dos ejes (campo clipped), con INPUT/SELECT/TEXTAREA exceptuados por escrito (Chromium les aplica overflow:clip en su hoja de agente). Mutacion de control acreditada: li{overflow:hidden;max-height:96px} hace fallar 3 de las 4 pruebas. Antes no medido: (`overflowX`/`overflowY` por elemento), que es la otra mitad del Then de @s42 |
