...
class TestStorage(unittest.TestCase):
    ...
    def test_save_serializes_human_readable_utf8(self) -> None:
        # Contrato: el almacén es JSON legible por humanos, en UTF-8 sin
        # escapar y con sangría de 2 espacios. Fija el formato exacto (mata los
        # mutantes de ensure_ascii=False -> True y de indent=2 -> otro valor).
        notes = [{"id": 1, "title": "café", "body": "ñandú",
                  "created_at": "2026-01-01T00:00:00+00:00"}]
        storage.save(notes, self.path)
        with open(self.path, "r", encoding="utf-8") as f:
            text = f.read()
        self.assertIn("café", text)          # UTF-8 crudo, no é
        self.assertIn("ñandú", text)
        self.assertNotIn("\\u", text)         # no hay escapes ascii
        lines = text.splitlines()
        self.assertEqual(lines[1], "  {")     # sangría de exactamente 2 espacios
