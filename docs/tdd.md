...
> "Do you let it write all tests up front, then code or single test followed
> by code (TDD)?" — La respuesta de este arnés: **single test followed by
> code**. Un test a la vez. Nunca toda la batería por delante.
...
El efecto: nunca tienes código sin un test que lo justifique, ni un test que
no esté empujando código real. El alcance no se infla.
...
- **ROJO** — el test deriva del siguiente escenario `@s` del `.feature`.
  Verifícalo fallando de verdad (`bin/harness test`). Un test que pasa a la
  primera no demuestra nada: ajústalo o sospecha del montaje.
- **VERDE** — la implementación **mínima**. Está permitido hacer trampa
  (devolver una constante) si aún no hay test que lo desmienta. El siguiente
  ciclo forzará la generalización. Esto es deliberado.
...
Un escenario con varias aristas (p. ej. "lista vacía imprime 0" y "tres notas
imprime 3") puede necesitar dos ciclos para forzar la generalización.

## Trazabilidad obligatoria

Al cerrar, cada `@s` debe estar cubierto por al menos un test concreto. El
`tdd_craftsman` escribe el mapa en `progress/tdd_<name>.md`:
...
> El runner de tests concreto (unittest, Vitest, `node --test`, go test…) lo
> declara `harness.config.json` → `commands.test`. La disciplina es la misma
> en todos los stacks.
