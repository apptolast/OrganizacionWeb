...
const MUTANTS = [
  ...
  // ------------------------------- racimo 5: la pantalla de GitLab, donde estaba el defecto
  [
    5,
    "screen 116 LogicalOperator instanceof || code",
    "screen",
  ...
  [
    5,
    "screen 117 CE code === CONNECTORS_DISABLED -> true",
    "screen",
    `        error instanceof GitlabConnectorError &&\n        error.code === "CONNECTORS_DISABLED"`,
    `        error instanceof GitlabConnectorError &&\n        true`,
  ],
  [
    5,
    "screen 319 StringLiteral not_connected -> ''",
    "screen",
    `{showPanel && connection && connection.status !== "not_connected" ? (`,
    `{showPanel && connection && connection.status !== "" ? (`,
  ],
  [
    5,
    "screen 319 CE status !== not_connected -> true",
    "screen",
    `{showPanel && connection && connection.status !== "not_connected" ? (`,
    `{showPanel && connection && true ? (`,
  ],
  [
    5,
    "screen 319 LogicalOperator showPanel || connection",
    "screen",
    `{showPanel && connection && connection.status !== "not_connected" ? (`,
    `{(showPanel || connection) && connection.status !== "not_connected" ? (`,
  ],
  ...
  [
    5,
    "screen 299 LogicalOperator ultimo && -> ||",
    "screen",
    `  const showPanel = !disabled && !loading && Boolean(connection) && !replacing;`,
    `  const showPanel = (!disabled && !loading && Boolean(connection)) || !replacing;`,
  ],
  [
    5,
    "screen 443 StringLiteral region viva vacia",
    "screen",
    `            : ""}`,
    `            : "Stryker was here!"}`,
  ],
  [
    5,
    "screen 303 UnaryOperator tabIndex main -> +1",
    "screen",
  ...
  [
    5,
    "screen 304 UnaryOperator tabIndex h1 -> +1",
    "screen",
  ...
  [
    5,
    "screen 461 UnaryOperator tabIndex recibo -> +1",
    "screen",
    `      ref={boxRef}\n      tabIndex={-1}`,
    `      ref={boxRef}\n      tabIndex={+1}`,
  ],
  [
    5,
    "screen 338 ArrowFunction onChange del selector",
    "screen",
  ...
  [
    5,
    "screen 74 StringLiteral selected inicial",
    "screen",
    `  const [selected, setSelected] = useState("");`,
    `  const [selected, setSelected] = useState("Stryker was here!");`,
  ],
  [
    5,
    "screen 166 CE current -> true",
    "screen",
    `      setSelected((current) => current || (open[0]?.id ?? ""));`,
    `      setSelected((current) => true || (open[0]?.id ?? ""));`,
  ],
  [
    5,
    "screen 166 StringLiteral sin proyectos",
    "screen",
    `      setSelected((current) => current || (open[0]?.id ?? ""));`,
    `      setSelected((current) => current || (open[0]?.id ?? "Stryker was here!"));`,
  ],
  [
    5,
    "screen 207 CE guarda de importacion -> false",
    "screen",
  ...
  [
    5,
    "screen 207 LogicalOperator importing && !selected",
    "screen",
    `    if (importing || !selected) return;`,
    `    if (importing && !selected) return;`,
  ],
  [
    5,
    "screen 141 BooleanLiteral focusReplace no se apaga",
    "screen",
  ...
  [
    5,
    "screen 146 BooleanLiteral focusReceipt no se apaga",
    "screen",
  ...
  [
    5,
    "screen 151 BooleanLiteral focusHeading no se apaga",
    "screen",
    `      focusHeading.current = false;`,
    `      focusHeading.current = true;`,
  ],

  // ---------------------------------------------------- racimo 6: el catalogo y su cliente
  [
    6,
    "catalogClient 48 CE super(code) -> true",
    "catalogClient",
    `    super(typeof body.code === "string" ? body.code : "CATALOG_ERROR");`,
    `    super(true ? body.code : "CATALOG_ERROR");`,
  ],
  [
    6,
    "catalogClient 48 CE super(code) -> false",
    "catalogClient",
    `    super(typeof body.code === "string" ? body.code : "CATALOG_ERROR");`,
    `    super(false ? body.code : "CATALOG_ERROR");`,
  ],
  [
    6,
    "catalogClient 48 EqualityOperator typeof !== string",
    "catalogClient",
    `    super(typeof body.code === "string" ? body.code : "CATALOG_ERROR");`,
    `    super(typeof body.code !== "string" ? body.code : "CATALOG_ERROR");`,
  ],
  [
    6,
    "catalogClient 48 StringLiteral 'string' -> ''",
    "catalogClient",
    `    super(typeof body.code === "string" ? body.code : "CATALOG_ERROR");`,
    `    super(typeof body.code === "" ? body.code : "CATALOG_ERROR");`,
  ],
  [
    6,
    "catalogClient 48 StringLiteral CATALOG_ERROR -> ''",
    "catalogClient",
    `    super(typeof body.code === "string" ? body.code : "CATALOG_ERROR");`,
    `    super(typeof body.code === "string" ? body.code : "");`,
  ],
  [
    6,
    "catalogClient 58 CE typeof code !== string -> false",
    "catalogClient",
  ...
  [
    6,
    "catalogClient 71 CE (null || instant) -> true",
    "catalogClient",
    `    !(value.lastActivityAt === null || instant(value.lastActivityAt))`,
    `    !(true)`,
  ],
  [
    6,
    "catalogClient 84 CE longitud del catalogo -> false",
    "catalogClient",
  ...
  [
    6,
    "catalogClient 103 ObjectLiteral { signal } -> {}",
    "catalogClient",
    `const response = await apiRequest(CATALOG_URL, { signal });`,
    `const response = await apiRequest(CATALOG_URL, {});`,
  ],
  [
    6,
    "catalogClient 102 throwIfAborted parada 1 -> ;",
    "catalogClient",
    `  signal.throwIfAborted();\n  const response = await apiRequest(CATALOG_URL, { signal });`,
    `  ;\n  const response = await apiRequest(CATALOG_URL, { signal });`,
  ],
  [
    6,
    "catalogClient 104 throwIfAborted parada 2 -> ;",
    "catalogClient",
    `  signal.throwIfAborted();\n  if (response.status !== 200) return failure(response);`,
    `  ;\n  if (response.status !== 200) return failure(response);`,
  ],
  [
    6,
    "catalogClient 107 throwIfAborted parada 3 -> ;",
    "catalogClient",
  ...
  [
    6,
    "catalogo 106 CE instanceof Error -> false",
    "catalog",
  ...
  [
    6,
    "catalogo 106 StringLiteral 'code' -> ''",
    "catalog",
    `          error instanceof Error && "code" in error`,
    `          error instanceof Error && "" in error`,
  ],
  [
    6,
    "catalogo 117 CallExpression controller.abort() -> ;",
    "catalog",
    `      live = false;\n      controller.abort();`,
    `      live = false;\n      ;`,
  ],
  [
    6,
    "catalogo 115 BlockStatement limpieza -> {}",
    "catalog",
    `    return () => {\n      live = false;\n      controller.abort();\n    };`,
    `    return () => {};`,
  ],
  [
    6,
    "catalogo 122 UnaryOperator tabIndex main -> +1",
    "catalog",
  ...
  [
    6,
    "catalogo 123 UnaryOperator tabIndex h1 -> +1",
    "catalog",
    `      <h1 ref={heading} tabIndex={-1}>`,
    `      <h1 ref={heading} tabIndex={+1}>`,
  ],
  [
    6,
    "catalogo 146 StringLiteral separador -> ''",
    "catalog",
    `        </span>{" "}`,
    `        </span>{""}`,
  ],
