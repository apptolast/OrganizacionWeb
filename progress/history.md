...
## 2026-09-10 — feature 27 `github_connector`: carril de cierre

Carril `claude/github-connector` de la noche del 9 al 10 de septiembre. Entra al turno con el juez ya pronunciado —**APPROVED condicionado**, `progress/judge_github_connector_cierre.md`— y con dos deudas: una regresión de E2E y la puerta de mutación de frontend. Bitácora completa del carril en `progress/cierre_github_connector.md`.

- **spec_partner / gherkin_author**: sin cambios de alcance. Los 42 escenarios de `features/github_connector.feature` siguen siendo el contrato. Se alinean dos filas caducadas (`:146` y `:165-166`): el recibo de importación pasa de once campos con `repository` a doce con `source` y `projectPath`, porque `features/additional_connectors.feature:242` (@s20, feature 29) exige que el recibo de GitHub y el de GitLab tengan las mismas claves. Cuando dos contratos se contradicen gana el posterior, y el anterior se corrige por escrito en vez de silenciarse.
- **tdd_craftsman, ciclo 1 — la regresión que tenía la CI en rojo**: cuatro pruebas de E2E no veían ningún contador en «Resultado de la importación». La causa estaba entre el caso de uso y la vista, no en el `TRUNCATE` ni en el caso de uso: `decodeReceipt` del cliente de frontend exigía el juego de claves **exacto** anterior y rechazaba de plano el recibo real con «Confirmación incompatible», así que el componente caía en el `catch` y la sección entera dejaba de existir. Rojo acreditado por ejecución antes del arreglo. Verde: doce claves, tipo con `source`/`projectPath` y `source === "github"` exigido. Commit `a1b0d20`.
- **tdd_craftsman, ciclo 2 — la puerta de mutación**: 25 pruebas nuevas contra los nueve racimos más densos de supervivientes, entre ellos el escuchador de Escape que el dictamen exigía nominalmente. **Producción intacta**: sólo cambian ficheros de prueba y ninguna prueba anterior se relaja. Commit `ba97280`.
- **Verificación, no razonamiento**: no se ejecutó Stryker (lo corre el orquestador en serie). En su lugar, `scripts/verificar-mutantes-github-connector.mjs` aplica cada mutante superviviente al fichero de producción, ejecuta la suite y restaura: **43 de 43 mueren**, con el detalle prueba a prueba en `progress/verificacion_mutantes_github_connector.json`. El de `integrations-index.tsx` y los cuatro nuevos del cliente se comprobaron aparte, a mano.
- **Corrección al parte anterior, descubierta ejecutando**: el superviviente de `github-connector.tsx:266` no es el ternario del `catch` sino la guarda `live(controller)` (columnas 266:11-27). La receta con `vi.mock` que quedaba apuntada no habría matado nada. Gana la ejecución.
- **judge**: condiciones no relativas a la mutación, revisadas una a una. La deuda del `CHECK` de `V25` estaba ya cerrada por `V30__github_connector_ciphertext_bounds.sql` (29-283) con dos pruebas de persistencia. El `forget()` que interpolaba el propietario en SQL ya no existe: hoy es un `TRUNCATE … CASCADE`. El `240_000` sin derivar del presupuesto de E2E pasa a derivarse del andamiaje.
- **E2E, sobre pila real en el puerto 18096**: 15/15 en `e2e/github-connector.spec.mjs` y 1/1 en `e2e/github-connector-native-zoom.spec.mjs`. Los dieciséis recorridos de la feature, verdes.
- **mutation_tester**: pendiente del orquestador. Previsión declarada para contrastar: de 445/589 (75,55 %) a ≈489/589 (**≈83,0 %**), es decir 44 muertes nuevas frente a las 27 que hacían falta.
- **Resultado**: `in_progress`. La feature **no** se marca `done` desde el carril: falta la campaña de mutación medida y `bin/harness init` verde, los dos en manos del orquestador.
