...
Refuerzo posterior a 16 casos: contexto 407ccbf (merge previo abortado sin pérdida; detach autorizado). @s25 valores PROJECT/TASK con changeValues real, lectura previa y advisory retenido, pg_blocking_pids acredita espera; writer confirma valor/ID y el import devuelve409 sin recibo ni eventos. Inicialmente GREEN 4b1ee9, 2/2, XML/log values25 preservados. No se atribuye este alcance a los16 anteriores.

@s26 valores PROJECT/TASK: import retiene transacción, SaveCustomFieldValues real espera y rechaza revisión compuesta antigua; IDs/revisiones/valor importado y recibo quedan íntegros, cero eventos. Inicialmente GREEN44be49, 2/2; XML/log values26 preservados.

@s18 snapshot real PROJECT/TASK: se retiene la lectura del archivo tras adquirir snapshot PG (backend_xmin presente). Un writer real confirma esquema y valores dentro de una transacción mientras preview sigue abierta; preview conserva ambos recuentos idénticos del estado anterior. Otra preview posterior detecta409; datos del writer permanecen, cero recibos/eventos. Inicialmente GREEN8e97c9, 2/2. No se exige bloqueo al writer: MVCC permite su commit; la barrera y Future pendiente acreditan solapamiento, pg_blocking_pids se usa en los otros dos grupos con espera real. XML/log snapshot18 preservados.

Regresión final ampliada:22/22 cero fallos/errores/omitidos y SpotlessJavaCheck EXIT0,29s. Sólo cambian ImportConcurrencyTest y esta bitácora; contexto407ccbf. Conservados log/exit/XML finales y originales anteriores. No PIT ni prueba global. Cobertura añadida específicamente valores@s25–26 y snapshot@s18, sin reinterpretar los16 previos.
