Feature: Filtrar notas por fecha
  Como usuario quiero ver lo que apunté desde una fecha dada para revisar mi
  actividad reciente sin listar todo el archivo.
  ...
  Scenario: Una nota creada exactamente en la fecha dada entra (límite inclusivo)
    ...
    When ejecuto "notes since 2026-05-01"
  ...
  @s6
  Scenario: Una fecha con rango imposible es un error
    Given un almacén con notas
    When ejecuto "notes since 2026-13-40"
    Then stderr contiene un mensaje sobre la fecha
    And el código de salida es 1
