...
`GET /api/v1/projects/{projectId}/tasks/{id}/status` devuelve exactamente status, completedAt y updatedAt, junto con el ETag fuerte `"task:<uuid>:<versión>"` obtenido del mismo snapshot SQL. `PUT` sobre la misma ruta admite sólo status literal pending o completed y exige If-Match. La revisión es propia de la tarea. Una precondición ausente produce 428 PRECONDITION_REQUIRED; formato inválido, 400 VALIDATION_ERROR; versión anterior de una tarea propia, 412 TASK_CONFLICT. Las rutas validan primero los identificadores, después la precondición y después JSON. La privacidad precede a la comparación de versión.

El cambio incrementa versión y confirma tarea, historia y outbox en una transacción. La fecha UTC usa microsegundos y nunca retrocede respecto a updatedAt anterior; completar fija completedAt y reabrir lo limpia. Un no-op con revisión vigente conserva cuerpo, fechas y ETag sin escrituras. Todos los estados de proyecto permiten estas transiciones, que conservan el proyecto y las demás tareas. DTO8 mantiene sus campos y acepta pending/completed en las lecturas; la creación sigue devolviendo pending.

V9 añade versión y fecha de finalización a tasks y crea task_status_history con FK compuesta al proyecto/tarea y unicidad por tarea/versión. El adaptador usa `FOR NO KEY UPDATE OF t` bajo READ_COMMITTED: serializa cambios de la tarea y permite KEY SHARE de la FK al crear un hijo concurrente. Las tres escrituras deben afectar una fila; un fallo o supresión revierte todas. La historia permanece independiente de la retención de outbox.

`GET /api/v1/projects/{projectId}/tasks/{id}/history` devuelve items y nextCursor, veinte transiciones por versión descendente. Cada entrada contiene id, fromStatus, toStatus y occurredAt. El cursor opaco contiene projectId, taskId y taskVersion, exige base64url canónica y BIGINT positivo; no depende del reloj ni de versiones consecutivas. Las rutas conservan sesión, CSRF para PUT, origen y no-store.
