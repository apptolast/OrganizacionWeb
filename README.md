...
El inicio, pausa, reanudación y cierre de sesiones están cerrados y fusionados. El cierre permite guardar avance y siguiente paso, conserva el tiempo trabajado y recupera su recibo mediante una URL estable. El aviso de fin y la ampliación deliberada también están cerrados y fusionados: [su dictamen](progress/judge_end_time_notification_final.md) conserva la validación y sus límites. El fin previsto no cierra automáticamente la sesión ni completa la tarea. El MVP todavía no está desplegado.

Historial está implementado y validado localmente: [dictamen de cierre](progress/judge_history_final.md). `/historial` reúne reservas, cambios de planificación, finalización/reapertura de tareas e inicio/cambios de sesión. Ofrece filtros por categoría y fechas UTC, entrada desde proyecto o tarea, detalles y paginación por URL. Conserva los recibos históricos y utiliza los nombres actuales de proyecto/tarea. La lectura no depende de conservar los eventos del outbox. Consulta el [plan de entrega](docs/mvp-delivery-plan.md) para distinguir implementación, pruebas pendientes y despliegue.

Pausa y reanudación están integradas mediante la [PR14](https://github.com/apptolast/OrganizacionWeb/pull/14). En **Sesión de trabajo**, **Pausar** detiene la acumulación del tiempo neto y **Reanudar** continúa la misma sesión. El fin previsto no se desplaza. El tiempo mostrado corresponde a la última actualización: **Actualizar estado de la sesión** solicita una nueva consulta. Si no se puede confirmar un cambio, **Comprobar cambio** recupera el recibo guardado; una consulta fallida no revoca una confirmación anterior.
...
- [Guía de uso del MVP](docs/mvp-user-guide.md): planificar, trabajar, cerrar y consultar el historial.
