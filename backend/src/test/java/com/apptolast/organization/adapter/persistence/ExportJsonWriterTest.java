...
class ExportJsonWriterTest {
  ...
  void s1_emptyAccountProducesTheClosedDocumentAndExactDownloadMetadata() throws Exception {
    var file = new ExportJsonWriter().empty("dueño", Instant.parse("2026-09-08T01:02:03.123456789Z"));
    ...
    assertThat(json.properties()).extracting(java.util.Map.Entry::getKey)
    ...
    var names = java.util.List.of("projects", "tasks", "taskStatusHistory", "availability", "plannedBlocks",
    ...
    assertThat(json.path("data").properties()).extracting(java.util.Map.Entry::getKey).containsExactlyElementsOf(names);
    assertThat(json.path("counts").properties()).extracting(java.util.Map.Entry::getKey).containsExactlyElementsOf(names);
    for (var name : names) {
