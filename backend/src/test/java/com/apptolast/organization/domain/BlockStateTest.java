...
class BlockStateTest {
  ...
  void s6_revisionStateAndExhaustionProtectCancellationInOrder(long version,long expected,String status,String code) {
    var start=LocalDateTime.parse("2030-01-07T10:00");
    var request=new BlockRequest("Meta",start,start.plusHours(1),"UTC",ZoneOffset.UTC,ZoneOffset.UTC,false);
    var time=new ResolvedBlockTime(start.toInstant(ZoneOffset.UTC),start.plusHours(1).toInstant(ZoneOffset.UTC),ZoneOffset.UTC,ZoneOffset.UTC,60);
    var block=new PlannedBlock(UUID.randomUUID(),UUID.randomUUID(),UUID.randomUUID(),request,time,Instant.EPOCH);
  ...
  void s12_s13_cancellationPreservesLastBlockWhenClockMovesBack() {
    ...
    var request=new BlockRequest("Meta",start,start.plusHours(1),"UTC",ZoneOffset.UTC,ZoneOffset.UTC,false);
    var time=new ResolvedBlockTime(start.toInstant(ZoneOffset.UTC),start.plusHours(1).toInstant(ZoneOffset.UTC),ZoneOffset.UTC,ZoneOffset.UTC,60);
    var block=new PlannedBlock(UUID.randomUUID(),UUID.randomUUID(),UUID.randomUUID(),request,time,Instant.parse("2030-01-06T10:00:00Z"));
  ...
  void s1_originalCreationDefinesInitialStateWithoutNewFacts() {
    var start = LocalDateTime.parse("2030-01-07T10:00");
    var request = new BlockRequest("Preparar borrador", start, start.plusHours(1), "UTC", ZoneOffset.UTC, ZoneOffset.UTC, false);
    var time = new ResolvedBlockTime(start.toInstant(ZoneOffset.UTC), start.plusHours(1).toInstant(ZoneOffset.UTC), ZoneOffset.UTC, ZoneOffset.UTC, 60);
    var block = new PlannedBlock(UUID.randomUUID(), UUID.randomUUID(), UUID.randomUUID(), request, time, Instant.parse("2030-01-06T10:00:00Z"));
