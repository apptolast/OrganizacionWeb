...
@Component
public final class PostgresProjectEditing implements ProjectEditing {
  private final JdbcTemplate jdbc;
  private final TransactionTemplate transaction;
  private final PostgresProjectQueries queries;
  private final ObjectMapper json;

  public PostgresProjectEditing(
      JdbcTemplate jdbc,
    ...
    try {
      return transaction.execute(
          status -> {
            ...
            int updated =
                jdbc.update(
                    "UPDATE projects SET name=?,description=?,updated_at=?,version=? WHERE owner_id=? AND id=? AND version=?",
                    project.name(),
                    project.description(),
  ...
  private String serialize(ProjectUpdated event) {
    try {
