...
Secreto generado por el servidor, mostrado una única vez y guardado cifrado con AES-256-GCM, nonce aleatorio de 12 bytes y AAD igual al `id` del propietario junto al `id` del endpoint —**enmienda del 10 de septiembre de 2026, ratificada por el propietario (R10)**: atar el secreto a propietario **y** recurso impide que un endpoint reasignado descifre el secreto anterior, y es lo que el código hace desde siempre; la línea decía sólo el endpoint y era falsa leída al pie—, bajo la clave de entorno `APP_CONNECTOR_KEY` (propiedad `app.connectors.key`, base64 de exactamente 32 bytes). Un hash no sirve porque la firma HMAC exige el secreto en claro en cada envío. Clave **ausente**: la aplicación arranca —**enmienda del 10 de septiembre de 2026, ratificada por el propietario (R11)**: decía «ausente o mal formada», y una clave **presente pero inválida** no deja arrancar ni debe hacerlo. La ausencia significa «webhooks deshabilitados»; una clave rota es un error del operador y tiene que verse al instante, no seis horas después cuando falle el primer envío—, `audit.workerError("CONFIGURATION_ERROR")`, el worker es no-op y las operaciones que necesitan clave responden 503 `CONNECTORS_DISABLED`. El secreto en claro sólo existe en memoria durante la respuesta de creación y durante la firma; nunca en logs, listados, export, sessionStorage/localStorage ni mensajes de error.
...
Ruta privada `/webhooks`, H1 «Webhooks», entrada de navegación tras «API para integraciones» sin desplazar Hoy. Estado vacío orientador: explica qué es un webhook, la firma y el límite de cinco. Formulario: URL (`type="url"`), descripción, casillas por tipo de evento con «Seleccionar todos» y ayuda escrita que **enlaza** la guía pública de verificación de firma (hoy `docs/webhooks.md`; **historia de esta línea**: prometía el enlace y era **falsa** —el formulario no contenía ninguno—, así que la mañana del 10 de septiembre de 2026 se corrigió el **documento** y no el producto, razonando que ningún `@s` pedía el enlace. El propietario mandó cerrarlo por el otro lado: el formulario **enlaza de verdad** desde el `@s43` de `features/webhooks.feature`, alcanzable con Tab, con nombre accesible que dice de qué es y con los 44 px del `@s42`. El contrato pide el **enlace**, no la **ruta**: clavar `docs/webhooks.md` en un oráculo sería frágil y no mata mutantes, y la guía puede mudarse de sitio sin que el usuario pierda nada. El `@s43` está **pendiente de contrafirma** del propietario; ver `progress/enlace_docs_webhooks.md`); el botón «Crear webhook» se deshabilita durante la petición. Tras 201 el secreto se muestra una vez, sólo en memoria, con selección manual y botón «Copiar» sin escritura automática; se oculta al desmontar, cambiar de propietario o cerrar sesión, y ninguna respuesta tardía lo publica a otra identidad. Respuesta de red incierta: mensaje explícito, sin reintento automático, botón «Actualizar lista».
