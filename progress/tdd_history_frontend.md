...
## UI40 — enlaces de navegación y foco

Root detectó que la captura se limitaba a apply/retry/paginación. Se reforzó únicamente el caso existente Limpiar filtros con foco en encabezado tras desaparecer el enlace: RED e6a334, history_ui_40_red.log. Captura local de enlaces Historial trasladada al main, limitada a href /historial o /historial? y al enlace que conserva foco; mantiene exclusión de clic no primario/modificadores y el listener que anula foco movido deliberadamente. Se retira la captura duplicada del nav. No cambios de otros casos ni nuevas matrices.

History30/30 GREEN3764aa, history_ui_40_green.log. Formato/check, ESLint y tipos EXIT0 298b9e. No regresión global durante init root ni campañas. Los rangos de soporte siguen válidos porque History se selecciona completo. Nuevo manifest puntual history_ui_focus_freeze.json; los informes/manifiestos anteriores conservan su corte histórico. CSS no cambiado por este ajuste.
