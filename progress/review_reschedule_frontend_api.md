...
## Corrección de formato tras CI — alcance exclusivo del test API

CI PR1/run34041579720 falló en init por Prettier sobre reschedule-api.test.ts (log del coordinador a1c3f9); no por tests. El check focal local con Prettier3.9.6 reprodujo el warning5ef41a sobre el archivo confirmado en cf5e3e3. Se aplicó --write únicamente a ese test: cuatro cadenas vi.fn().mockResolvedValue de los ciclos7/9/10/15 cambian distribución de líneas y comas finales; el último test19 no cambia. No hubo cambio de EOL: el contenido Git y el resultado tienen cero CRLF. No hay configuración Prettier encontrada ni paquete Prettier raíz que explique otra regla por cwd.

Causa comprobada: el archivo versionado no coincidía con el formato canónico del formateador instalado en esas cuatro expresiones. La evidencia disponible no permite reconstruir por qué b139b1 se registró como formato verde; no se atribuye sin prueba a la escritura del test19 ni a Windows/CRLF. Se conserva la discrepancia con el registro anterior.

Check focal y diff-check posteriores GREEN3e9ef3. Comparación AST TypeScript de HEAD y resultado, descartando posiciones/trivia, confirma árbol sintáctico idéntico66c640; conserva todos los literales, llamadas y oráculos. Comparaciones previas de tokens/texto emitido resultaron diferentes por comas/trivia y no se usaron como prueba de cambio semántico. Suite focal19/19 GREEN66c640, sin suites globales ni cambios API/configuración. Este es un fix de formato autorizado, no un RED de producto. Autoría limitada al test y este append; commit/push a cargo del coordinador.
